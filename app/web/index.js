...
(partials = require('express-partials')),
    ...
    (twitterRss = process.env.CONSUMER_KEY
        ? require('rss-twitter')(
...
app.use(function(req, res, next) {
    ...
    if (!req.session.first_visit) {
        //this is the user's first visit, send it to the tracking url
...
var homepage_test = {
    ...
    d: { background: 'happy.jpg', title: 'Schedule regular HTTP requests to your webapp.' }, //based on B (test winner), make wording more obvious
};
    ...
    if (errors) {
        ...
        User.findOne({ email: req.body.email }, function(err, user) {
            if (user) {
                User.update(
                    { _id: user._id },
                    { $addToSet: { attr: { name: 'stripeToken', value: token } } },
                    function() {
...
app.get('/ln', function(req, res, next) {
    ...
    if (!req.query.t || !req.query.r) {
        ...
        User.findOne({ apikey: req.query.t }, function(err, user) {
            if (!user) {
                //user not found for this apiKey
                res.redirect('/login');
            }
