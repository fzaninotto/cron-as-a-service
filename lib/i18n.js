...
function guessLanguage(request) {
  if (typeof request === 'object') {
    var cloudflare_header = request.headers['HTTP_CF_IPCOUNTRY '],
    ...
    if(cloudflare_header) {
        switch(cloudflare_header){
            case 'UK': language_header = 'en-gb'; break;
            case 'GB': language_header = 'en-gb'; break;
            case 'AT': language_header = 'en-au'; break;
        }
    }
