            ...
            function() {
                try {
                    ...
                    switch (job.method) {
                        ...
                        case 'get':
                            body = job.params != null && job.params.length > 0 ? job.params : {};
                            break;
                        //otherwise use requestBody if present
                        default:
                            body = job.requestBody != null && job.requestBody.length > 0 ? job.requestBody : {};
                            break;
                    }

                    req.send(body).end(function(response) {
                        job.responses.push({
                            date: new Date(),
...
function postResponseJobs(job, response) {
    ...
    if (job.responseEmail && job.responseEmail.match(/\@/g)) {
        ...
        var mailOptions = {
            ...
            text:
                'You are receiving this because you (or someone else) have requested the results of your request to ' +
                job.url +
                ' at ' +
                new Date() +
                ' .\n\n' +
...
function sendAlarm(job, response) {
    try {
        /*cio.track(
            job.user,
            'alarm',
            { response: response.raw_body, statusCode: response.statusCode, url: job.url, expression: job.expression },
            function(err, res) {
                if (err != null) {
                    console.log('ERROR', err);
                }
